...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("radar_ftp_daemon.log"), logging.StreamHandler()],
)
...
async def run_production_daemon():
    ...
    daemon = FTPDaemon(config, on_file_downloaded=processor.process_bufr_file, logger=logger)
